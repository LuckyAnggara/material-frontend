<template>
  <div class="card flex bg-neutral flex-col lg:w-1/3 w-full">
    <div class="card-body shadow-xl rounded-xl">
      <h2 class="card-title mb-2 text-2xl">Data Unit Item</h2>
      ...
      <div class="flex mt-2 md:overflow-visible overflow-y-auto">
        <table class="table table-compact w-full">
          ...
          <tbody>
            ...
            <template v-else>
              <tr v-if="itemStore.itemUnits.length == 0">
                <td colspan="3" class="text-center">Tidak ada data</td>
              </tr>
              <tr
                v-else
                v-for="(item, index) in itemStore.itemUnits"
                :key="item.id"
              >
    ...
    newUnit() {
      this.$swal
        .fire({
          ...
          backdrop: true,
          preConfirm: (value) => {
            return this.itemStore.storeUnitData(value).then((resp) => {
              if (resp.status == 200) {
                return resp
              }
            })
            ...
            if (result.value.status == 200) {
              this.$swal
                .fire({
                  icon: 'success',
                  title: 'Berhasil',
                  text: 'Unit item baru berhasil di tambahkan',
                  backdrop: true,
    ...
    onDelete(id, index) {
      this.$swal
        .fire({
          ...
          backdrop: true,
          preConfirm: (value) => {
            return this.itemStore.deleteUnitData(id, index).then((resp) => {
              if (resp.status == 200) {
                return resp
              }
            })
