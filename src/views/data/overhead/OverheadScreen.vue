...
export default {
  ...
  methods: {
    newOverhead() {
      this.$swal
        .fire({
          title: 'Input data Overhead',
          html: `<div class="grid gap-y-2 justify-items-center w-full">
                <input class="input input-bordered w-3/4" id="name" placeholder="Nama" />
          ...
          preConfirm: () => {
            ...
            return this.itemStore.storeOverheadData(value).then((resp) => {
              if (resp.status == 200) {
                return resp
              }
            })
            ...
            if (result.value.status == 200) {
              this.$swal
                .fire({
                  icon: 'success',
                  title: 'Berhasil',
                  text: 'Overhead item baru berhasil di tambahkan',
                  backdrop: true,
    ...
    onDelete(id, index) {
      this.$swal
        .fire({
          ...
          backdrop: true,
          preConfirm: (value) => {
            return this.itemStore.deleteOverheadData(id, index).then((resp) => {
              if (resp.status == 200) {
                return resp
              }
            })
