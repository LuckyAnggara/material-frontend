<template>
  <div class="card flex bg-neutral flex-col lg:w-1/3 w-full">
    <div class="card-body shadow-xl rounded-xl">
      <h2 class="card-title mb-2 text-2xl">Data Tipe Item</h2>
      ...
      <div class="flex mt-2 md:overflow-visible overflow-y-auto">
        <table class="table table-compact w-full">
          ...
          <tbody>
            ...
            <template v-else>
              <tr v-if="itemStore.itemTypes.length == 0">
                <td colspan="3" class="text-center">Tidak ada data</td>
              </tr>
              <tr
                v-else
                v-for="(item, index) in itemStore.itemTypes"
                :key="item.id"
              >
                <td class="text-center">{{ from + index }}</td>
                <td @dblclick="checkTrue(index)">
    ...
    newType() {
      this.$swal
        .fire({
          ...
          backdrop: true,
          preConfirm: (value) => {
            return this.itemStore.storeTypeData(value).then((resp) => {
              if (resp.status == 200) {
                return resp
              }
            })
            ...
            if (result.value.status == 200) {
              this.$swal
                .fire({
                  icon: 'success',
                  title: 'Berhasil',
                  text: 'Tipe item baru berhasil ditambahkan',
                  backdrop: true,
    ...
    onDelete(id, index) {
      this.$swal
        .fire({
          ...
          backdrop: true,
          preConfirm: (value) => {
            return this.itemStore.deleteTypeData(id, index).then((resp) => {
              if (resp.status == 200) {
                return resp
              }
            })
